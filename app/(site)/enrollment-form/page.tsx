...
const validationSummaryMessage =
  "Please review the highlighted fields below and correct any missing or invalid information before continuing.";

const formSteps: readonly FormStep[] = [
  {
    title: "Student Information",
    label: "Student Info",
    reassurance: "This should take just a few minutes.",
    fieldNames: [
      "session_enrolled",
      "payment_email_used_for_stripe_deposit",
      "laptop_access_for_program",
      "laptop_operating_system",
      "student_full_name",
      "preferred_name",
      "date_of_birth",
      "student_age",
      "student_grade_2026_2027",
      "student_tshirt_size",
      "student_school",
    ],
  },
  {
    ...
    label: "Family Info",
    reassurance: "Contact details help us keep communication and pickup smooth.",
    fieldNames: [
      ...
      "parent_guardian_2_phone_number",
      "home_address",
      "city",
      "state",
      "zip_code",
      ...
      "additional_authorized_pickup_adults",
    ],
  },
  {
    title: "Medical / Safety",
    label: "Safety",
    reassurance: "Please share anything that will help us safely support your student during hands-on activities.",
    fieldNames: [
      "allergies",
      "medical_conditions",
      "current_medications",
  ...
  },
  {
    ...
    label: "Background",
    reassurance: "This helps us tailor projects and coaching to your student.",
    fieldNames: [
      "previous_robotics_experience_level",
      "previous_robotics_programs[]",
      "previous_robotics_programs_other_details",
      "programming_experience[]",
      "programming_experience_other_details",
      "cad_experience[]",
      "cad_experience_other_details",
      "summer_build_goal",
      "robotics_background_notes",
      "photo_media_permission",
      "referral_source",
      "referrer_name",
    ],
  },
  {
    title: "Final Review",
    label: "Final Review",
    reassurance: "Final step - a few standard acknowledgements and safety agreements.",
    fieldNames: [
      "ack_info_accuracy",
      ...
      "liability_waiver_medical_consent_agreement",
      "signature_parent_guardian_full_name",
      "signature_date",
    ],
  },
] as const;
...
const validateControls = (
  ...
  const controls = getNamedControls(form).filter((control) =>
    allowedFieldNames ? allowedFieldNames.has(control.name) : true,
  );
...
export default function EnrollmentFormPage() {
  ...
  const [lastValidatedStep, setLastValidatedStep] = useState<number | null>(null);
  const [hasAttemptedSubmit, setHasAttemptedSubmit] = useState(false);
  const formRef = useRef<HTMLFormElement>(null);

  const totalSteps = formSteps.length;
  const currentStepDefinition = formSteps[currentStep];

  const fieldToStepMap = useMemo(() => {
    const mapping: Record<string, number> = {};
    formSteps.forEach((step, stepIndex) => {
      step.fieldNames.forEach((fieldName) => {
        mapping[fieldName] = stepIndex;
      });
    });
    return mapping;
  }, []);
  ...
  useEffect(() => {
    ...
    const activeStepHeading = form.querySelector<HTMLElement>(`[data-step-header="${currentStep}"]`);
    if (activeStepHeading) {
      activeStepHeading.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [currentStep]);

  const focusFirstInvalidField = (firstInvalidField: FieldControl | null) => {
    if (!firstInvalidField) {
      return;
    }

    const targetStep = fieldToStepMap[firstInvalidField.name];
    ...
    window.setTimeout(() => {
      ...
      const candidate = form.elements.namedItem(firstInvalidField.name);
      let controlToFocus: FieldControl | null = null;

      if (candidate instanceof RadioNodeList) {
        const firstRadio = candidate[0];
        if (firstRadio instanceof HTMLInputElement) {
          controlToFocus = firstRadio;
        }
      } else if (
        ...
        controlToFocus = candidate;
      }

      if (controlToFocus) {
        controlToFocus.scrollIntoView({ behavior: "smooth", block: "center" });
        window.setTimeout(() => controlToFocus?.focus({ preventScroll: true }), 120);
      }
    }, 180);
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (currentStep < totalSteps - 1) {
      ...
      const { errors: stepErrors, firstInvalidField } = validateControls(
        form,
        currentStepDefinition.fieldNames,
      );

      if (Object.keys(stepErrors).length > 0) {
        setFieldErrors((prev) => ({ ...prev, ...stepErrors }));
        setShowValidationSummary(true);
        setLastValidatedStep(currentStep);
      ...
      setFieldErrors((prev) => {
        ...
        currentStepDefinition.fieldNames.forEach((fieldName) => {
          delete nextErrors[fieldName];
        });
        return nextErrors;
      });
      setShowValidationSummary(false);
      setLastValidatedStep(null);
    ...
    const { errors: validationErrors, firstInvalidField } = validateControls(form);
  ...
  const handleFormFieldUpdate = () => {
    ...
    if (hasAttemptedSubmit) {
      const { errors: nextErrors } = validateControls(form);
    ...
    if (lastValidatedStep !== null) {
      const { errors: nextStepErrors } = validateControls(form, formSteps[lastValidatedStep].fieldNames);
      setFieldErrors((prev) => {
        const merged = { ...prev };
        formSteps[lastValidatedStep].fieldNames.forEach((fieldName) => {
          delete merged[fieldName];
        });
        return { ...merged, ...nextStepErrors };
      });
  ...
  const handleStepNext = () => {
    ...
    const { errors: stepErrors, firstInvalidField } = validateControls(form, currentStepDefinition.fieldNames);
    ...
    <section className="pt-6 md:pt-8 pb-16">
      <div className="mx-auto max-w-5xl px-6 text-white">
        ...
        {submitted ? (
          ...
          <>
            <p className="mt-3 text-base md:text-lg text-neutral-200 leading-relaxed">
              Thank you for reserving a seat in Camp Asimov. Please complete the guided enrollment below so we can finalize your student&apos;s enrollment and prepare for the summer session.
            </p>
            <p className="mt-2 text-sm md:text-base text-neutral-300">
              This form takes about 10-15 minutes to complete. It includes the standard contact, safety, and background information needed to support your student well.
            </p>
            <p className="mt-2 text-sm md:text-base text-neutral-300">
        ...
        {submitted ? (
          ...
          <form
            ...
            noValidate
            className="enrollment-form mt-8 rounded-2xl border p-6 md:p-8"
            style={{ borderColor: ink.line, background: "#0f141f" }}
          >
            <style jsx>{`
              ...
              .step-panel-active {
                animation: stepEnter 220ms ease-out;
              }
            ...
            <div className="mb-8 space-y-5">
              <div className="rounded-xl border px-4 py-4 md:px-5" style={{ borderColor: ink.line, background: "#111521" }}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm md:text-base font-semibold text-neutral-100">
                    Step {currentStep + 1} of {totalSteps} - {currentStepDefinition.title}
                  </p>
                  <p className="text-xs md:text-sm text-neutral-300">{currentStepDefinition.reassurance}</p>
                </div>
                <div className="mt-3 h-2 w-full rounded-full bg-[#1c2434]" aria-hidden="true">
                  <div
                    className="h-full rounded-full transition-all duration-300"
                    style={{ width: `${progressPercent}%`, background: "linear-gradient(90deg, #5ec8ff 0%, #8fd7ff 100%)" }}
                  />
                </div>
                <div className="mt-3 grid grid-cols-5 gap-2 text-[11px] md:text-xs text-neutral-300">
                  {formSteps.map((step, stepIndex) => {
                    const isComplete = stepIndex < currentStep;
                    const isCurrent = stepIndex === currentStep;
                    const hasErrors = step.fieldNames.some((fieldName) => hasFieldError(fieldName));
                    ...
                    return (
                      <div
                        ...
                        style={{
                          borderColor: hasErrors ? "#f87171" : isCurrent ? "#5ec8ff" : ink.line,
                          background: isCurrent ? "rgba(94, 200, 255, 0.12)" : "rgba(255, 255, 255, 0.01)",
                        }}
                      >
                        {step.label}
                      </div>
            ...
            <div hidden={currentStep !== 0} className={currentStep === 0 ? "step-panel-active space-y-10" : "space-y-10"}>
              <section>
                <h2 data-step-header="0" className="text-xl md:text-2xl font-semibold">Session + Student Information</h2>
                <p className="mt-2 text-sm md:text-base text-neutral-300">
                  We&apos;ll start with your student&apos;s details and session logistics.
                </p>
            ...
            <div hidden={currentStep !== 1} className={currentStep === 1 ? "step-panel-active space-y-10" : "space-y-10"}>
              <section>
                <h2 data-step-header="1" className="text-xl md:text-2xl font-semibold">Parent / Guardian + Emergency Contact</h2>
                <p className="mt-2 text-sm md:text-base text-neutral-300">
                  We&apos;ll use this information for day-to-day communication and emergency coordination.
                </p>
                <div className="mt-6 grid md:grid-cols-2 gap-5">
                  <div>
            ...
            <div hidden={currentStep !== 2} className={currentStep === 2 ? "step-panel-active space-y-10" : "space-y-10"}>
              <section
                className={sectionErrors.medicalSafety ? "rounded-lg border-l-2 border-red-400 pl-4" : undefined}
              >
                <h2 data-step-header="2" className={`text-xl md:text-2xl font-semibold ${sectionErrors.medicalSafety ? "text-red-200" : ""}`}>
                  Medical / Safety + Learning Support
                </h2>
                <p className="mt-2 text-sm md:text-base text-neutral-300">
                  Please share anything that will help us safely support your student during hands-on activities.
                </p>
              ...
              <section>
                <h3 className="text-lg md:text-xl font-semibold">Learning / Support Information</h3>
                <div className="mt-5">
            ...
            <div hidden={currentStep !== 3} className={currentStep === 3 ? "step-panel-active space-y-10" : "space-y-10"}>
              <section>
                <h2 data-step-header="3" className="text-xl md:text-2xl font-semibold">Robotics / Technical Background + Media Permission</h2>
                <p className="mt-2 text-sm md:text-base text-neutral-300">
                  Share your student&apos;s background so we can balance challenge, confidence, and team fit.
                </p>
              ...
              <section>
                <h3 className="text-lg md:text-xl font-semibold">Photo / Media Permission</h3>
              ...
              <section>
                <h3 className="text-lg md:text-xl font-semibold">How did you hear about Camp Asimov?</h3>
            ...
            <div hidden={currentStep !== 4} className={currentStep === 4 ? "step-panel-active space-y-10" : "space-y-10"}>
              <section>
                <h2 data-step-header="4" className="text-xl md:text-2xl font-semibold">Final Acknowledgements + Waiver + Signature</h2>
                <p className="mt-2 text-sm md:text-base text-neutral-300">
                  Final step - a few standard acknowledgements and safety agreements.
                </p>
                <div className="mt-5 rounded-xl border px-4 py-4 text-sm md:text-base text-neutral-200" style={{ borderColor: ink.line, background: "#111521" }}>
                  Please take a final moment to review and confirm the required acknowledgements below. Submission will complete your student&apos;s enrollment.
                </div>
              </section>
              ...
              >
                <h3 className={`text-lg md:text-xl font-semibold ${sectionErrors.requiredAcknowledgements ? "text-red-200" : ""}`}>
              ...
              >
                <h3 className={`text-lg md:text-xl font-semibold ${sectionErrors.liabilityWaiver ? "text-red-200" : ""}`}>
                ...
                <div className="mt-4 rounded-lg border px-4 py-4 text-sm md:text-base text-neutral-100 leading-relaxed space-y-4" style={{ borderColor: ink.line, background: "#111521" }}>
                  ...
                  <p>
                    I hereby release and hold harmless Camp Asimov LLC, its staff, instructors, assistants, and affiliates from any and all liability, claims, or damages arising from my child&apos;s participation in the program, except in cases of gross negligence or willful misconduct.
                  </p>
                  <p>
              ...
              <section>
                <h3 className="text-lg md:text-xl font-semibold">Signature</h3>
            ...
            <div className="mt-10 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Button
                ...
                style={{ borderColor: ink.line, color: "#e5e7eb", background: "#111521" }}
              >
                Back
              </Button>
